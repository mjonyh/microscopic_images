% TikZ schematic: FFT Analysis Pipeline
...
\draw[arrow] (feat) -- ++(0,-0.6) -| (radial);
\draw[arrow] (feat) -- ++(0,-0.6) -| (azimuth);
\draw[arrow] (feat) -- ++(0,-0.6) -| (scalar);
...
\draw[arrow] (input) |- ++(0,-2.5) -| (enhance);
\draw[arrow] (enhance) -- (filter);
\draw[arrow] (filter) -- (unet);

% Brace
\draw[decorate, decoration={brace, amplitude=5pt, mirror}]
    ($(input.south west)+(-0.2,-0.3)$) -- ($(feat.south east)+(0.2,-0.3)$)
    node[midway, below=0.4cm, label] {\textbf{FFT Feature Extraction}};

\draw[decorate, decoration={brace, amplitude=5pt, mirror}]
    ($(enhance.south west)+(-0.2,-0.3)$) -- ($(unet.south east)+(0.2,-0.3)$)
    node[midway, below=0.4cm, label] {\textbf{Enhancement \& Segmentation Pipeline}};

\end{tikzpicture}
\caption{\textbf{Computational framework overview.} Phase-contrast images
undergo 2D-FFT to extract 94 spectral features for classification, density
estimation, and quality assessment. For segmentation, images pass through
physics-informed enhancement (DeBCR, PI-DDPM) followed by bandpass filtering
