...
[x] 0.6  Verify output directory exists
      Command: test -d outputs && echo "OK" || echo "MISSING"
      Expected: "OK"

[x] 0.7  Verify git repo initialized and remote set
...
Steps:
[x] 1.1  Load each TIFF image as float64 array
[x] 1.2  Subtract mean (remove DC component)
[x] 1.3  Apply 2D Hanning window to reduce edge artifacts
[x] 1.4  Compute 2D FFT → fftshift → power spectrum = |FFT|²
[x] 1.5  Compute radial power profile (azimuthally averaged, 100 bins)
[x] 1.6  Extract spectral features per image:
...
[x] 1.7  Match features to COCO cell counts (25% subset = 808 images)
[x] 1.8  Compute Pearson correlation: cell_count vs each spectral feature
[x] 1.9  Generate 4-panel plot:
...
Steps:
[x] 2.1  Compute radial power spectrum per image (200 bins for fine resolution)
[x] 2.2  Find peak frequency (skip DC bin) → peak_period = 1/freq (in pixels)
[x] 2.3  Compute spectral centroid → mean_period = 1/centroid
[x] 2.4  Validate: correlate peak_period vs sqrt(mean_area) from COCO
[x] 2.5  Generate 4-panel plot:
...
Steps:
[x] 3.1  Compute 2D FFT power spectrum per image
[x] 3.2  Compute azimuthal power profile (36 angle bins, 0-180°)
[x] 3.3  Compute isotropy score: 1 - (std/mean) of azimuthal profile
        High isotropy (~1) = clean/isotropic image
        Low isotropy (~0) = directional artifacts (halo, shading)
[x] 3.4  Compute low-frequency fraction (first 10% of radial bins)
        High value = background shading artifact
[x] 3.5  Compute high-frequency fraction (last 20% of radial bins)
        High value = noise floor
[x] 3.6  Generate 4-panel plot:
...
Steps:
[x] 4.1  Build per-image feature vector (94 features total):
...
[x] 4.2  Assemble feature matrix X: shape (3727, 94), labels y: shape (3727,)
[x] 4.3  Define 3 classifiers:
...
[x] 4.4  Run 5-fold stratified cross-validation for each classifier
[x] 4.5  Report mean ± std accuracy for each classifier
[x] 4.6  Select best classifier, generate cross-validated predictions
[x] 4.7  Compute 8×8 confusion matrix (row-normalized)
[x] 4.8  Generate classification report (precision, recall, F1 per class)
[x] 4.9  Generate 2-panel plot:
...
Steps:
[x] 5.1  For each annotated image, segment raw image with Otsu thresholding
[x] 5.2  Compute IoU between raw segmentation and COCO ground truth (bbox proxy)
[x] 5.3  Apply 5 bandpass filter configurations:
...
[x] 5.4  For each filtered image, segment with Otsu + compute IoU vs ground truth
[x] 5.5  Select best filter (highest IoU) per image
[x] 5.6  Compute improvement = IoU_filtered - IoU_raw
[x] 5.7  Generate 4-panel plot:
...
Steps:
[x] 6.1  Group images by well_id (CellLine_Plate_Well) → 22 wells
[x] 6.2  Sort each well's images by timepoint (parse hours from filename)
[x] 6.3  For each image, compute spectral features (centroid, bandwidth, power bands)
[x] 6.4  Compute centroid rate of change (derivative between consecutive frames)
[x] 6.5  Bin time series into 6-hour windows, compute mean ± std per cell line
[x] 6.6  Detect mitosis-like events:
...
[x] 7.1  Verify all 12 output files exist:
...
[x] 7.2  Review all 6 PNG figures visually for correctness

[x] 7.3  Review CSV outputs for expected row counts and column names

[x] 7.4  Record key findings in README.md:
...
[x] 7.5  Initial commit and push to GitHub:
      cd ~/git/livecell
      git add .
      git commit -m "Initial: LIVECell FFT analysis project setup"
      git push -u origin main

[x] 7.6  Verify GitHub repo shows all files:
